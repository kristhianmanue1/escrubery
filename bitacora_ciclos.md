...
| 2026-08-17 | F3 | T3 — Eliminados + version_actual + qwen semanal | 0.5 | 0.75 | +0.25 | T3a: eliminados = fuente sandbox ausentes del `--help` → evento `deprecacion` + vigencia vencida (detectó artefacto `claude --help` de test F1); T3c: `--version` del sandbox alimenta `cli_productos.version_actual` (opencode 1.18.15, claude 2.1.197, codex 0.147.0, qwen 0.15.10); T3b: qwen-code entra a la ventana semanal F3 de la vigilancia. **Bug intermedio corregido**: dedup solo-contra-sandbox re-flaggeaba comandos de ficha como nuevos y el onConflict pisaba fecha/hash de filas `docs_oficial` (procedencia restaurada con re-ingesta; evento `ev-f3-opencode-1786990268241` quedó espurio en la cadena — append-only, se anota aquí); fix: dedup contra TODAS las filas, eliminados solo contra sandbox; idempotencia verificada (2 corridas 0/0); 79/79 | `vigilancia_diaria.sh`: paso F3 tras pollers (auto-build de imagen si ausente, Docker caído → omite con nota, fallo de build → exit 2); corrida real: 3 diarios introspectados + exit 10 (alertas conocidas); 79/79; check_sizes OK | Ficha `qwen-code.json` (Apache-2.0, hash de LICENSE real, oficial/qwenlm); ToS curado (6/9); mig `010` (CHECK +qwen); generador +`qwen:dashscope` → ficha 41 modelos (25 con precio); `./scripts/consultar oficialidad` → 9 CLIs; `consultar modelo qwen qwen3-coder-plus` → ventana 997952; `refrescar_litellm.sh` corrido; Dockerfile.qwen-code (build pendiente de Docker); vigilancia semanal +qwen-code |
