...
- **H1 — Verificación propia automatizada:** `npm test` verde con specs de Evidentia (cadena/firma/verificar/clasificador), golden del shape post-errata y JCS validado contra test vectors RFC 8785. [pendiente]
- **H2 — Operación continua:** pipeline CI definido y verde, vigilancia diaria (pollers + alertas) instalada con log de corrida real, y caducidad `vigente_hasta` corregida y activa con degradación visible. [pendiente]
