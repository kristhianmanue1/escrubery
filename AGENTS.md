...
- **En curso (2026-08-22):** H8 — memoria enforcada AN-KLA (hooks claude-code + plugin opencode, `scripts/hooks-spike/`, `.opencode/plugins/ankla_gate.ts`). T1–T7b verificados por ejecución, incluida la confirmación viva en runtime Bun (`opencode run` → `SI-INYECTADA`, inyección por llamada LLM). Pendiente: cierre de H8 por decreto del Mediador.
